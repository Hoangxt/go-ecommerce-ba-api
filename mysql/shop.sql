...
CREATE TABLE `shopdev_user` (
    user_id int NOT NULL AUTO_INCREMENT COMMENT 'user id', user_name VARCHAR(255) NULL DEFAULT NULL COMMENT 'user name', user_email VARCHAR(255) NULL DEFAULT NULL COMMENT 'email user', PRIMARY KEY (`user_id`)
...
CREATE TABLE `shopdev_role` (
    role_id int NOT NULL COMMENT 'role id', role_name VARCHAR(255) NULL DEFAULT NULL COMMENT 'role name', role_description VARCHAR(255) NULL DEFAULT NULL COMMENT 'role description', PRIMARY KEY (`role_id`)
) ENGINE = INNODB CHARACTER SET = utf8mb4;
...
INSERT INTO
    shopdev_role
VALUES (
        1, 'admin', 'read,update,delete,create'
    );
...
CREATE TABLE `shopdev_menu` (
    menu_id int NOT NULL AUTO_INCREMENT COMMENT 'menu_id', menu_name VARCHAR(255) NULL DEFAULT NULL COMMENT 'name menu', menu_pid VARCHAR(255) NULL DEFAULT NULL COMMENT 'name menu', menu_path VARCHAR(255) NULL DEFAULT NULL COMMENT 'path', PRIMARY KEY (`menu_id`)
...
INSERT INTO
    shopdev_menu
VALUES (
        11, 'Dong ho', '11035788', '/Đồng-Hồ-cat.11035788'
    );

INSERT INTO
    shopdev_menu
VALUES (
        12, 'may tinh', '11035954', '/Máy-Tính-Laptop-cat.11035954'
    );

INSERT INTO
    shopdev_menu
VALUES (
        13, 'thoi trang nam', '11035567', '/Thời-Trang-Nam-cat.11035567'
    );
...
CREATE TABLE `shopdev_role_menu` (
    role_id int NOT NULL COMMENT 'role id', menu_id int NOT NULL COMMENT 'menu id', PRIMARY KEY (`role_id`, `menu_id`)
) ENGINE = INNODB CHARACTER SET = utf8mb4;
...
CREATE TABLE `shopdev_user_role` (
    user_id int NOT NULL COMMENT 'user id', role_id int NOT NULL COMMENT 'role id', PRIMARY KEY (`user_id`, `role_id`)
) ENGINE = INNODB CHARACTER SET = utf8mb4;
...
WHERE
    menu_id in (
        SELECT menu_id
        FROM shopdev_role_menu
        WHERE
            role_id in (
                SELECT role_id
                FROM shopdev_user_role
                WHERE
                    user_id = 1
            )
    );
...
-- cach 2
SELECT *
FROM shopdev_menu mn, (
        SELECT rome.menu_id
        FROM
            shopdev_user_role usro, shopdev_role_menu rome
        WHERE
            usro.role_id = rome.role_id
            AND usro.user_id = 1
    ) t
WHERE
    mn.menu_id = t.menu_id;
